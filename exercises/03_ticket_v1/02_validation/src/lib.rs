...
impl Ticket {
    ...
    fn new(title: String, description: String, status: String) -> Self {
        ...
        // if !(status.eq("To-Do") || status.eq("In Progress") || status.eq("Done")) { panic!("Only `To-Do`, `In Progress`, and `Done` statuses are allowed") }
        if status != "To-Do" && status != "In Progress" && status != "Done" { panic!("Only `To-Do`, `In Progress`, and `Done` statuses are allowed") }
        Self {
            title,
            description,
            status,
